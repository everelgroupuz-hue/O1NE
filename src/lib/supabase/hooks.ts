  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .rpc('get_order_messages', { p_order_id: orderId });
...
export const useMarkMessagesRead = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (params: { order_id: string; sender_id: string }) => {
...
export const useUnreadMessageCount = (senderId: string) => {
  return useQuery({
    queryKey: ['unread-messages', senderId],
    queryFn: async () => {
      const { data, error } = await supabase
        .rpc('get_unread_message_count', { p_sender_id: senderId });
      if (error) throw error;
      return data ?? 0;
    },
...
export const useAllProductAnalytics = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.rpc('get_all_product_analytics');
      if (error) throw error;
      return data ?? [];
    },
  });
