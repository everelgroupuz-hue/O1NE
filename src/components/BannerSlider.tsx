        ...
        {banners.map((b, i) => {
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col justify-end px-4 sm:px-6 pb-5 sm:pb-6">
              ...
              <p
                className="text-text-inverse font-extrabold text-lg sm:text-2xl leading-tight drop-shadow-lg max-w-[80%] sm:max-w-[75%]"
              ...
              {b.link_url && b.link_label && (
                <button
                  onClick={handleCta}
                  className="mt-3 sm:mt-4 self-start flex items-center gap-1.5 sm:gap-2 px-4 sm:px-5 py-2 sm:py-2.5 bg-white text-text rounded-xl text-[11px] sm:text-xs font-bold hover:bg-white/90 active:scale-95 transition-all shadow-float"
                >
      ...
      {banners.length > 1 && (
        <>
          <button
            ...
            className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 z-20 w-8 h-8 rounded-full bg-black/25 hover:bg-black/45 backdrop-blur-sm flex items-center justify-center transition-colors"
          >
            <ChevronLeft className="w-4 h-4 text-text-inverse" />
          </button>
          <button
            onClick={() => { goTo(current + 1); startTimer(); }}
            className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 z-20 w-8 h-8 rounded-full bg-black/25 hover:bg-black/45 backdrop-blur-sm flex items-center justify-center transition-colors"
          >
