...
export const ProductCard = memo(({ product, language, favoriteIds: favoriteIdsProp, hideStockBadge }: ProductCardProps) => {
  ...
  return (
    <div
      ...
      className="card-premium overflow-hidden cursor-pointer group relative"
      style={{ isolation: 'isolate' }}
    >
      {/* Image */}
      <div className="relative aspect-[4/5] bg-surface-muted overflow-hidden rounded-t-2xl">
        ...
        {!hideStockBadge && product.stock > 0 && product.stock < 5 && (
          <div className="absolute top-2 left-2 bg-accent text-text-inverse text-2xs font-bold px-2 py-0.5 rounded-lg z-10">
        ...
        {product.stock > 0 && (
          <button
            ...
            className={`absolute bottom-2 right-2 w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200 active:scale-90 z-10 ${
              justAdded
                ? 'bg-green-500 text-text-inverse'
                : 'bg-accent text-text-inverse group-hover:bg-accent-hover opacity-100 sm:opacity-0 sm:group-hover:opacity-100 sm:translate-y-2 sm:group-hover:translate-y-0'
            }`}
          >
      ...
      {/* Info */}
      <div className="p-3.5">
        <h3 className="text-sm font-semibold text-text line-clamp-1 mb-1">
