...
export const Cart = () => {
    ...
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center px-4 py-24">
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-surface-muted flex items-center justify-center mb-4 sm:mb-5">
            <ShoppingBag className="w-8 h-8 sm:w-9 sm:h-9 text-text-tertiary" />
          </div>
          <h2 className="text-base sm:text-lg font-bold text-text mb-1 sm:mb-1.5">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 rounded-xl bg-accent hover:bg-accent-hover text-text-inverse text-sm font-semibold transition-colors"
          >
  ...
  return (
    <Layout showBottomNav={false}>
      <div className="bg min-h-screen pb-36">
        {/* Header */}
          ...
          {items.map((item) => {
            ...
            return (
              <div
                ...
                className={`bg-surface rounded-2xl p-3 shadow-sm border ${
                  stockError ? 'border-orange-300 dark:border-orange-700' : 'border-border-subtle'
                }`}
              >
                <div className="flex gap-3">
                  {/* Image */}
                  <div
                    className="w-16 h-16 sm:w-20 sm:h-20 bg-surface-muted rounded-xl overflow-hidden flex-shrink-0 cursor-pointer"
                  >
                    {item.image ? (
                      <img
                        src={item.image}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      {item.size && (
                        <span className="text-xs px-2 py-0.5 rounded-md bg-surface-muted text-text-secondary">
                          {t('size')}: {item.size}
                        </span>
                      )}
                      {item.color && (
                        <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-md bg-surface-muted text-text-secondary">
                          <span
                    ...
                    <div className="flex items-center justify-between mt-2.5">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-surface-muted flex items-center justify-center active:scale-90 transition disabled:opacity-30"
                          disabled={item.quantity <= 1}
                        >
                          <Minus className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-text-secondary dark:text-text-tertiary" />
                        </button>
                        <span className="w-7 sm:w-8 text-center text-xs sm:text-sm font-bold text-text">
                          {item.quantity}
                        </span>
                        <button
                          onClick={() => handleUpdateQty(item.productId, item.quantity, item.quantity + 1, item.size, item.color)}
                        ...
                        >
                          <Plus className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-text-secondary dark:text-text-tertiary" />
                        </button>
                        <button
                          onClick={() => setConfirmRemove({ productId: item.productId, size: item.size, colorHex: item.color?.hex })}
                          className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center ml-0.5 sm:ml-1 text-text-tertiary hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 active:scale-90 transition"
                        >
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 pb-safe">
        <div className="bg-white/95 dark:bg-surface-elevated/95 backdrop-blur-md border-t border-border px-4 pt-3 pb-4 shadow-elevated">
          {/* Summary */}
          ...
          <button
            ...
            className="w-full py-3.5 rounded-xl bg-accent hover:bg-accent-hover active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed text-text-inverse text-sm font-semibold transition-all flex items-center justify-center gap-2"
          >
            {hasErrors
      ...
      {confirmRemove && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-6">
          <div className="bg-surface rounded-2xl p-5 w-full max-w-xs shadow-xl">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-surface-muted text-text text-sm font-medium active:scale-95 transition"
              >
                {t('cancel')}
              </button>
              <button
                onClick={confirmRemoveItem}
                className="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-text-inverse text-sm font-medium active:scale-95 transition"
              >
                {t('delete')}
              </button>
            </div>
