    ...
    <div className="min-h-screen bg">
      <header className="sticky top-0 z-40 bg-surface border-b border-border">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-text">Купоны</h1>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-3">
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm text-text-secondary">Нет купонов</p>
          </div>
        ) : (
          coupons.map((coupon) => (
            <div key={coupon.id} className="bg-surface rounded-2xl p-4 border border-border flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-accent flex items-center justify-center">
                  <Tag className="w-5 h-5 text-text-inverse" />
                </div>
                <div>
                  <p className="font-bold text-text font-mono">{coupon.code}</p>
              ...
              <div className="flex items-center gap-2">
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${coupon.is_active ? 'bg-success/10 text-success' : 'bg-surface-muted text-text-secondary'}`}>
                  {coupon.is_active ? 'Активен' : 'Выкл'}
                </span>
                <button onClick={() => handleDelete(coupon.id)} className="p-2 rounded-lg hover:bg-danger/10 text-text-tertiary hover:text-danger transition">
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-surface rounded-t-2xl sm:rounded-2xl p-6 w-full max-w-md max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-text">Новый купон</h2>
              <button onClick={() => setShowCreate(false)} className="p-2 rounded-lg hover:bg-surface-muted"><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="text-xs font-semibold text-text-secondary mb-1 block">Код</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-text-secondary mb-1 block">Тип</label>
                ...
                <div>
                  <label className="text-xs font-semibold text-text-secondary mb-1 block">Значение</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-text-secondary mb-1 block">Мин. сумма</label>
                ...
                <div>
                  <label className="text-xs font-semibold text-text-secondary mb-1 block">Лимит всего</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-text-secondary mb-1 block">Лимит/пользователь</label>
                ...
                <div>
                  <label className="text-xs font-semibold text-text-secondary mb-1 block">Действует до</label>
